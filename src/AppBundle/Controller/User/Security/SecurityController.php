...
class SecurityController extends Controller
{
    ...
    public function resetAction(Request $request,AuthenticationUtils $authUtils, UserPasswordEncoderInterface $passwordEncoder, Connection $connection, EncoderFactory $encoderFactory)
    {
        ...
        {
            $error = SecurityController::setErrorCode($error, $error_count,'Sicherheitsfrage Fehler', 'Es wurde keine Sicherheitsfrage ausgewählt!');
            $bool_question = false;
            $error_count++;
        }
        ...
        {       
            ...
            if($res)
            {
                ...
                {
                    $error = SecurityController::setErrorCode($error, $error_count,'Passwort Fehler', 'Die Passwörter stimmen nicht überein!');
                    $bool_new_password = false;
                    $error_count++;
                }
            }
            ...
            {
                $error = SecurityController::setErrorCode($error, $error_count, 'Antwort Fehler', 'Die Antworten stimmen nicht überein!');
                $bool_answer = false;
                $error_count++;
            }
        }
